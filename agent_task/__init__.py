...
class C(BaseConstants):
    ...
    INSTRUCTIONS_TEMPLATE = __name__ + "/instructions_task.html"
    INSTRUCTIONS_TEMPLATE_2 = __name__ + "/instructions_bel.html"
...
class Player(BasePlayer):
    ...
    # Beliefs
    #--------------------------------------------------
    belief = models.IntegerField(min=0, max=100, initial=0)# these are the beliefs
...
def play_game(player: Player, message: dict):
    ...
    if message_type == "answer":
        ...
        if current.is_correct:
            ...
            player.num_failed += 1
        player.num_trials += 1
        player.payoff = player.num_correct * C.PIECE_RATE
...
def compute_payoff(player: Player):
    player.payoff = player.num_correct * 2
...
# to control time limit in the two repetitions (short and long)
    @staticmethod
...
page_sequence = [Introduction, Instructions_task2, Game, Results, Slider_BSR]
